...
(() => {
  ...
  function updateSessionChart(stats) {
    ...
    if (window.GptUi && window.GptUi.updateRealtimeChart) {
      window.GptUi.updateRealtimeChart('session-realtime-chart', successRate, {
        color: 'var(--ops-blue)',
    ...
    const runningEl = document.getElementById('session-chart-running');
    const errorEl = document.getElementById('session-chart-errors');
    const successEl = document.getElementById('session-rail-success');
  ...
  function renderJobs() {
    if (state.order.length === 0) {
      ...
      updateSessionChart({ running: 0, success: 0, error: 0, cancelled: 0 });
      return;
    }
  ...
  dom.btnRun.addEventListener('click', async () => {
    ...
    try {
      // Sync config
      const target = window.GptUi.modeToConcurrency(document.getElementById('session-concurrency-select').value);
  ...
  document.addEventListener('gpt:tab', (e) => {
    if (e.detail.tab === 'session') {
      const modeEl = document.getElementById('session-concurrency-select');
  ...
  document.addEventListener('gpt:mode-change', async (e) => {
    if (e.detail.tab !== 'session') return;
