...
(() => {
  ...
  const dom = {
    ...
    proxyToggle:   $('upi-proxy-toggle'),
    proxyInput:    $('upi-proxy-input'),
  ...
  function updateUpiChart(stats) {
    const finished = (stats.success || 0) + (stats.error || 0) + (stats.cancelled || 0);
    const successRate = finished ? ((stats.success || 0) / finished) * 100 : 0;
    const errorRate = finished ? ((stats.error || 0) / finished) * 100 : 0;
    ...
    if (window.GptUi && window.GptUi.updateRealtimeChart) {
      window.GptUi.updateRealtimeChart('upi-realtime-chart', successRate, {
        color: 'var(--ops-green)',
        displayValue: window.GptUi.fmtPercent ? window.GptUi.fmtPercent(successRate) : `${successRate.toFixed(1)}%`,
        label: 'UPI success rate',
        visualAmplitude: Math.max(1, successRate * 0.06),
      });
    }
    const runningEl = document.getElementById('upi-chart-running');
    const errorEl = document.getElementById('upi-chart-errors');
    const successEl = document.getElementById('upi-rail-success');
    if (successEl) successEl.textContent = String(stats.success || 0);
    if (runningEl) runningEl.textContent = String(stats.running || 0);
    if (errorEl) errorEl.textContent = String(stats.error || 0);

    const formatPercent = window.GptUi && window.GptUi.fmtPercent ? window.GptUi.fmtPercent : (value) => `${value.toFixed(1)}%`;
    const formatDuration = window.GptUi && window.GptUi.fmtDuration ? window.GptUi.fmtDuration : fmtDuration;
    const setText = (id, value) => {
      const el = document.getElementById(id);
      if (el) el.textContent = value;
    };
    setText('upi-metric-total-jobs', String(total));
    setText('upi-metric-total-note', total === 1 ? '1 job in queue' : `${total} jobs in queue`);
    setText('upi-metric-success-rate', formatPercent(successRate));
    setText('upi-metric-success-note', `${stats.success || 0} success`);
    const upiSuccessRate = document.getElementById('upi-metric-success-rate');
    const upiSuccessRing = upiSuccessRate
      ? upiSuccessRate.closest('.metric-card')?.querySelector('.metric-ring')
      : null;
    if (upiSuccessRing) upiSuccessRing.style.setProperty('--metric-ring', `${Math.max(0, Math.min(100, successRate))}%`);
    setText('upi-metric-active-runs', String(stats.running || 0));
    setText('upi-metric-active-note', (stats.running || 0) ? 'In progress' : 'Idle now');
    setText('upi-metric-avg-time', avgDuration == null ? '-' : formatDuration(avgDuration));
    setText('upi-metric-avg-note', durations.length ? `${durations.length} timed jobs` : 'No duration yet');
    setText('upi-metric-error-rate', formatPercent(errorRate));
    setText('upi-metric-error-note', `${stats.error || 0} failed`);
    if (window.GptUi && window.GptUi.updateRealtimeChart) {
      window.GptUi.updateRealtimeChart('upi-total-jobs-chart', total, {
        color: 'var(--ops-green)',
        displayValue: total,
        label: 'UPI total jobs',
        maxPoints: 32,
        visualAmplitude: Math.max(1, total * 0.08),
      });
      window.GptUi.updateRealtimeChart('upi-active-runs-chart', stats.running || 0, {
        color: 'var(--ops-blue)',
        displayValue: stats.running || 0,
        label: 'UPI active runs',
        maxPoints: 32,
        visualAmplitude: Math.max(1, (stats.running || 0) * 0.2),
      });
      window.GptUi.updateRealtimeChart('upi-avg-time-chart', avgDuration == null ? 0 : avgDuration, {
        color: 'var(--ops-blue)',
        displayValue: avgDuration == null ? '-' : formatDuration(avgDuration),
        label: 'UPI average job time',
        maxPoints: 32,
        visualAmplitude: avgDuration == null ? 1 : Math.max(1, avgDuration * 0.08),
      });
      window.GptUi.updateRealtimeChart('upi-error-rate-chart', errorRate, {
        color: 'var(--ops-red)',
        displayValue: formatPercent(errorRate),
        label: 'UPI error rate',
        maxPoints: 32,
  ...
  function renderJobs() {
    if (state.order.length === 0) {
      ...
      updateUpiChart({ running: 0, success: 0, error: 0, cancelled: 0 });
      return;
    }
    ...
    const html = state.order.map((id, idx) => {
      ...
      let errBadge = '';
      if (j.error === 'PAID') {
          errBadge = `<span class="badge" style="background-color: var(--success); color: white;">PAID</span>`;
      } else if (j.error) {
          errBadge = `<span class="upi-err-inline" title="${escHtml(j.error)}">${escHtml(j.error.slice(0, 60))}</span>`;
      }
  ...
  dom.btnRun.addEventListener('click', async () => {
    ...
    const upiProxy = dom.proxyInput.value.trim();
    const useProxy = dom.proxyToggle.checked;
    if (useProxy && !upiProxy) {
      dom.proxyInput.setAttribute('aria-invalid', 'true');
      dom.proxyInput.focus();
      await Dialog.alert({ message: 'Proxy đang bật. Hãy nhập UPI proxy hoặc tắt Use proxy.' });
      return;
    }
    ...
    try {
      const target = window.GptUi.modeToConcurrency(document.getElementById('upi-concurrency-select').value);
      const approveRetries = parseInt(dom.approveRetries.value, 10) || 500;
      const jobTimeoutRaw = dom.jobTimeout.value.trim();
      const jobTimeout = jobTimeoutRaw === "" ? 0 : (parseInt(jobTimeoutRaw, 10) || 1800);
      ...
      const config = await api('/api/upi/config', {
        method: 'POST',
        body: JSON.stringify({
      ...
      dom.proxyInput.removeAttribute('aria-invalid');
      await Settings.save('upi.proxy', config.proxy || null, window.GptUi.getAuthToken());
      await Settings.save('upi.use_proxy', !!config.use_proxy, window.GptUi.getAuthToken());
      await api('/api/upi/jobs', {
        method: 'POST',
        body: JSON.stringify({ combos, sessions }),
      });
    } catch (err) {
      if (/invalid UPI proxy/i.test(err.message || '')) {
        dom.proxyInput.setAttribute('aria-invalid', 'true');
      }
  ...
  dom.proxyInput.addEventListener('change', async () => {
    ...
    try {
      ...
      dom.proxyInput.setAttribute('aria-invalid', 'true');
      await Dialog.alert({ message: 'UPI proxy is invalid: ' + err.message });
    }
  });
  ...
  dom.proxyToggle.addEventListener('change', async () => {
    ...
    try {
      const config = await api('/api/upi/config', {
        method: 'POST', body: JSON.stringify({ use_proxy: useProxy }),
      });
      await Settings.save('upi.use_proxy', !!config.use_proxy, window.GptUi.getAuthToken());
    } catch (err) {
      dom.proxyToggle.checked = !useProxy;
      await Dialog.alert({ message: 'Không lưu được trạng thái UPI proxy: ' + err.message });
    }
  });
  ...
  document.addEventListener('gpt:tab', (e) => {
    if (e.detail.tab === 'upi') {
      const modeEl = document.getElementById('upi-concurrency-select');
      if (modeEl) modeEl.value = window.GptUi.concurrencyToMode(state.maxConcurrent);
    }
  });

  document.addEventListener('gpt:mode-change', async (e) => {
    if (e.detail.tab !== 'upi') return;
    const target = e.detail.concurrency;
    try {
      await api('/api/upi/config', {
        method: 'POST',
  ...
  api('/api/upi/config').then((cfg) => {
    ...
    dom.proxyInput.value = typeof cfg.proxy === 'string' ? cfg.proxy : '';
